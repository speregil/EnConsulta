...
public class ControladorMicrofono : MonoBehaviour {
	...
	void Start () {
		...
		nivelActual = 0;
		nombreUsuario = GameObject.Find("DataManager").GetComponent<DatosEstudiante>().usuario;
		nombreCaso = GameObject.Find("DataManager").GetComponent<DatosCasos>().nombreCaso;
		numGrabacion = 0;
	}
	...
	IEnumerator Grabando() {
		...
		SavWav.SaveList("Record/" + nombreUsuario + "_" + nombreCaso + "_" + nivelActual + "_" + numGrabacion, audList);
		record.interactable = true;
	...
	IEnumerator Reproduciendo() {
		WWW www = new WWW("file://" + Application.dataPath + "/Record/" + nombreUsuario + "_" + nombreCaso + "_" + nivelActual + "_" + numGrabacion + ".wav");
		yield return www;
	...
	private int NumeroGrabacion(){
		...
		int numTemp = -1;
		string prefijo = nombreUsuario + "_" + nombreCaso + "_" + nivelActual + "_";
